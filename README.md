...
[![TCP](https://img.shields.io/badge/TCP-RFC_9293-blue?style=flat-square&logo=ietf&logoColor=white)](https://www.rfc-editor.org/rfc/rfc9293)
[![HTTP/2](https://img.shields.io/badge/HTTP%2F2-RFC_9113-green?style=flat-square&logo=ietf&logoColor=white)](https://www.rfc-editor.org/rfc/rfc9113)
...
| [02. 리버스 프록시 — Nginx 내부 동작과 upstream 연결 관리](./load-balancing-and-proxy/02-reverse-proxy-nginx.md) | Nginx가 클라이언트 연결과 upstream 연결을 분리해서 관리하는 방식, `keepalive` 지시어로 upstream TCP 연결을 재사용하는 풀 관리, 버퍼링(Buffering)이 느린 클라이언트로부터 upstream 서버를 보호하는 원리, `access_log`에서 `upstream_response_time`과 `request_time` 차이 |
...
- [RFC 9113 — Hypertext Transfer Protocol Version 2 (HTTP/2)](https://www.rfc-editor.org/rfc/rfc9113)
